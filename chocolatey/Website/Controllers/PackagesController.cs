...
namespace NuGetGallery
{
    public partial class PackagesController : Controller
    {
        ...
        // NOTE: Intentionally NOT requiring authentication
        public virtual ActionResult MarkOutdated(string id, string version)
        {
            var package = packageSvc.FindPackageByIdAndVersion(id, version);
            ...
            var model = new PackageOutdatedViewModel
            {
                PackageId = id,
                PackageVersion = package.Version,
            };
        ...
        [HttpPost, ValidateAntiForgeryToken, ValidateSpamPrevention]
        public virtual ActionResult MarkOutdated(string id, string version, PackageOutdatedViewModel form)
        {
            if (!ModelState.IsValid)
            {
                return ReportAbuse(id, version);
            ...
            messageService.ReportPackageOutdated(from, package);

            TempData["Message"] = "Your note has been passed along. If you don't see anything from the owners, please follow up.";
            return RedirectToAction(MVC.Packages.DisplayPackage(id, version));
        }

        // NOTE: Intentionally NOT requiring authentication
        public virtual ActionResult MarkBroken(string id, string version)
        {
            var package = packageSvc.FindPackageByIdAndVersion(id, version);
            ...
            var model = new PackageBrokenViewModel
            {
                PackageId = id,
                PackageVersion = package.Version,
            };
        ...
        [HttpPost, ValidateAntiForgeryToken, ValidateSpamPrevention]
        public virtual ActionResult MarkBroken(string id, string version, PackageBrokenViewModel form)
        {
            if (!ModelState.IsValid)
            {
                return ReportAbuse(id, version);
